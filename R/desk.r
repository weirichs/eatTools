desk <- function(variable,na=NA, p.weights = NULL, na.rm = FALSE) {
         ...
         ret      <- do.call("rbind", lapply(variable, FUN = function ( y ) {
                     ...
                     if(isFALSE(Mis.weight)) {
                        Summe      <- sum( y * p.weights , na.rm = na.rm)
                        ...
                        t2         <- sum(p.weights^2 * ((na.omit(y) - Mittelwert)^2))
                        err        <- sqrt(t1*t2)                               ### https://en.wikipedia.org/wiki/Weighted_arithmetic_mean
                     }                                                          ### Abschnitt 'Bootstrapping validation'
                     dataFrame <- data.frame ( N = N, N.valid = N.valid, Missing = length(y) - length(na.omit(y)), Minimum = min(y, na.rm = na.rm), Maximum = max(y, na.rm = na.rm), Summe = Summe, Mittelwert = Mittelwert, std.err = err, sig = ifelse(length(table(y))==1, NA, t.test(x = y)$p.value), Median = median(y, na.rm = na.rm), Streuung = sqrt(Varianz), Varianz = Varianz , stringsAsFactors = FALSE )
                     return(dataFrame)} ))
